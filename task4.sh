...
sleep 2 && curl -s https://raw.githubusercontent.com/0xevoid/Inery4/main/main.sh | bash && sleep 5
...
StepBox "Data Setup Before Running Inery Testnet Task 4" 80 "-"

read -p "$(printf "\033[33mEnter your Inery Account Name:\033[0m ")" INERY_ACCOUNT
read -p "$(printf "\033[33mEnter your Node IP Address:\033[0m ")" NODE_URL
read -p "$(printf "\033[33mEnter your Github Username:\033[0m ")" GITHUB_ACCOUNT

pre_setup(){
  if which git > /dev/null; then
    printf "\033[33mGit Already Installed, No Need To Install Again\033[0m\n"
  else
    sudo apt install git -y
  fi
  
#  if which node > /dev/null; then
#    printf "\033[33mNodeJS Already Installed, No Need To Install Again😀\033[0m\n"
#  else
...
#  fi
  ...
  if which curl > /dev/null; then
    printf "\033[33mCurl Already Installed, No Need To Install Again😀\033[0m\n"
  else
    type -p curl >/dev/null || sudo apt install curl -y
  fi

  if which gh > /dev/null; then
    echo "\033[33mGithub CLI Already Installed, No Need To Install Again😀\033[0m\n"
  else
...
StepBox "Install the required software" 80 "-"
pre_setup
gh auth login
...
git_pr
printf "\n\n\033[32mInery Testnet Task 4 FINISHED!!! Check the URL above to make sure your pull request is already in Git Inery\033[0m\n\n\n"
